...
export function CheckpointUpdates({journey,shareToken}:{journey:PublicJourney;shareToken?:string}){
  ...
  async function toggle(){
    ...
    try{
      ...
      if(!config.publicKey)throw new Error('The site owner still needs to configure journey notifications. Updates remain available in this page.');
      ...
      setMessage(next?'Journey notifications enabled for this journey. You can close this page.':'Journey notifications disabled for this journey.');
  ...
  return <section className="checkpoint-card"><h2>{shareToken?'Progress without watching the map':'Your checkpoints'}</h2>
    ...
    {shareToken&&<><button className="button secondary full" onClick={toggle} disabled={busy}><Bell size={17}/>{busy?'Updating…':enabled?'Turn off journey notifications':'Enable journey notifications'}</button><p className="field-hint">Receive checkpoint, destination-reached, and confirmed-safe notifications. Internet and notification permission are required. Missed-check-in and SOS alerts currently appear in the tracking page.</p><p className="field-hint">On iPhone: add this app to your Home Screen to use push notifications.</p></>}
